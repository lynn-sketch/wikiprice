/* WikiPrice SVG icons — no emojis */
function WPIcon(name, size) {
  size = size || 18;
  const paths = {
    ...
    heartFilled: '<path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" fill="currentColor" stroke="currentColor"/>',
    ...
    free: '<rect x="3" y="8" width="18" height="4" rx="1"/><path d="M12 8V4"/><path d="M8 4h8"/>',
  ...
  return '<svg class="wp-icon" width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + p + '</svg>';
}

function WPIconRow(name, text, size) {
  return '<span class="icon-row">' + WPIcon(name, size) + '<span>' + text + '</span></span>';
}

function WPIconLabel(name, label, size) {
  return WPIcon(name, size) + ' <span>' + label + '</span>';
}
